{{-- resources/views/clientes/medidores/create.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Crear medidor en') }}
            <span class="font-normal"> {{ $ciudad->nombre }} / {{ $sector->sector }}</span>
        </h2>
    </x-slot>

    <div class="py-[5vh] px-[5vw] max-w-3xl mx-auto">
        <form method="POST"
              action="{{ route('medidores.store', [$ciudad, $sector]) }}">
            @csrf
            ...
            <div class="mb-6">
                ...
                >
                    <option value="1" {{ old('estado_medidor',1) == 1 ? 'selected' : '' }}>Activo</option>
                    <option value="0" {{ old('estado_medidor') === '0' ? 'selected' : '' }}>Inactivo</option>
                    <option value="2" {{ old('estado_medidor') === '2' ? 'selected' : '' }}>En revisión</option>
                </select>
            ...
            <div class="flex space-x-4">
                ...
                <a
                    href="{{ route('medidores.index', [$ciudad, $sector]) }}"
