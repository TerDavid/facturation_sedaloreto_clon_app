...
@endif
    <div x-data="clienteManager()" class="py-6 ">
        ...
        <div x-show="isOpen" x-cloak
            class="fixed inset-0 bg-black/50 bg-opacity-50x flex items-start justify-center pt-16 z-50">
            <div class="bg-white rounded-lg w-full max-w-2xl p-6 relative">
                ...
                <form :action="formAction" method="POST" @submit="prepareSubmit(event)">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <select name="sector_id" x-model="form.sector_id" @change="filterManzanas()"
                                x-bind:disabled="!sectores.length" class="w-full p-2 border rounded" required>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                        <div><label>Nombre</label><input name="nombre" x-model="form.nombre"
                                class="w-full p-2 border rounded" required></div>
                        <div><label>Dirección</label><input name="direccion" x-model="form.direccion"
                                class="w-full p-2 border rounded"></div>
                        <div><label>Teléfono</label><input name="telefono" x-model="form.telefono"
                                class="w-full p-2 border rounded"></div>
                        <div><label>Correo</label><input type="email" name="email" x-model="form.email"
                                class="w-full p-2 border rounded"></div>
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="inline-flex items-center">
                            <input type="hidden" name="crear_medidor" x-bind:value="form.crear_medidor ? 'true' : 'false'">
                            <input type="checkbox" x-model="form.crear_medidor"
                                class="form-checkbox">
                    ...
                    <div x-show="form.crear_medidor" class="space-y-4 mb-4">
                        ...
                        <div><label>Código Medidor</label><input name="medidor_codigo" x-model="form.medidor_codigo"
                                class="w-full p-2 border rounded" x-bind:required="form.crear_medidor"></div>
                        <div><label>Fecha instalación</label><input type="date" name="medidor_fecha_instalacion"
                                x-model="form.medidor_fecha_instalacion" class="w-full p-2 border rounded"></div>
                        <div><label>Ubicación detallada</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div x-show="form.crear_medidor">
                            ...
                            <select name="tarifa_id" x-model="form.tarifa_id" class="w-full p-2 border rounded"
                                x-bind:required="form.crear_medidor">
                                <option value="">→ Selecciona tarifa</option>
                                <template x-for="t in tarifas" :key="t.id">
                                    <option :value="t.id"
                                        x-text="`${t.categoria} – ${t.rango_min}–${t.rango_max ?? '∞'}`"></option>
                                </template>
                            </select>
                        </div>
                        <div x-show="!form.crear_medidor">
                            ...
                            <select name="consumo_sin_medidor_id" x-model="form.consumo_sin_medidor_id"
                                class="w-full p-2 border rounded" x-bind:required="!form.crear_medidor">
                                <option value="">→ Selecciona consumo</option>
                                <template x-for="c in consumos" :key="c.id">
                                    <option :value="c.id" x-text="`${c.categoria} – ${c.asignado_m3} m³/mes`">
                                    </option>
                                </template>
                            </select>
                        </div>
        ...
        function clienteManager() {
            return {
                ...
                filterSectores() {
                    ...
                    // );
                    this.sectores = []
                    axios.get(`/api/ciudades/${this.form.ciudad_id}/sectores`).then(response => {
                ...
                filterManzanas() {
                    ...
                    // );
                    this.manzanas = []
                    axios.get(`/api/sector/${this.form.sector_id}/manzanas`).then(response => {
                ...
                updateCode() {
                    ...
                    const d = new Date(),
                        ...
                        fecha = `${y}${m}${day}`;
                    this.form.code_suministro =
                        `${this.form.ciudad_id}-${this.form.sector_id}-${this.form.manzana_id}-${fecha}`;
                },
