...
<x-app-layout>
    ...
    <div x-data="clienteManager()" class="p-6 space-y-6">
      ...
      <div x-show="isOpen" x-cloak
        ...
        <div class="bg-white rounded-lg w-full max-w-2xl p-6 relative">
          ...
          <form :action="formAction" method="POST" @submit="prepareSubmit()">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
              <div><label>Nombre</label><input name="nombre" x-model="form.nombre" class="w-full p-2 border rounded" required></div>
              <div><label>Dirección</label><input name="direccion" x-model="form.direccion" class="w-full p-2 border rounded"></div>
              <div><label>Teléfono</label><input name="telefono" x-model="form.telefono" class="w-full p-2 border rounded"></div>
              <div><label>Correo</label><input type="email" name="email" x-model="form.email" class="w-full p-2 border rounded"></div>
            </div>

            {{-- Medidor --}}
            <div class="mb-4">
              <label class="inline-flex items-center">
                <input type="checkbox" name="crear_medidor" x-model="form.crear_medidor" class="form-checkbox">
                <span class="ml-2">Registrar también medidor</span>
              </label>
            </div>

            <div x-show="form.crear_medidor" class="space-y-4 mb-4">
              <div><label>Código Medidor</label><input name="medidor_codigo" x-model="form.medidor_codigo" class="w-full p-2 border rounded" required></div>
              <div><label>Fecha instalación</label><input type="date" name="medidor_fecha_instalacion" x-model="form.medidor_fecha_instalacion" class="w-full p-2 border rounded"></div>
              <div><label>Ubicación detallada</label><textarea name="ubicacion_detallada" x-model="form.ubicacion_detallada" class="w-full p-2 border rounded"></textarea></div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div x-show="form.crear_medidor">
                <label>Tarifa</label>
                <select name="tarifa_id" x-model="form.tarifa_id" class="w-full p-2 border rounded" required>
                  <option value="">→ Selecciona tarifa</option>
                  <template x-for="t in tarifas" :key="t.id">
                    <option :value="t.id" x-text="`${t.categoria} – ${t.rango_min}–${t.rango_max ?? '∞'}`"></option>
                  </template>
                </select>
              </div>
              <div x-show="!form.crear_medidor">
                <label>Consumo estándar (sin medidor)</label>
                <select name="consumo_sin_medidor_id" x-model="form.consumo_sin_medidor_id" class="w-full p-2 border rounded" required>
                  <option value="">→ Selecciona consumo</option>
                  <template x-for="c in consumos" :key="c.id">
                    <option :value="c.id" x-text="`${c.categoria} – ${c.asignado_m3} m³/mes`"></option>
                  </template>
                </select>
              </div>
            </div>
            ...
            <div class="mb-4">
              ...
              <input name="code_suministro" x-model="form.code_suministro" readonly class="w-full p-2 border rounded bg-gray-100 cursor-not-allowed">
            </div>

            {{-- Botones --}}
            <div class="flex justify-end space-x-2">
              <button type="button" @click="closeForm()" class="px-4 py-2 border rounded">Cancelar</button>
              <button type="submit" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded" x-text="mode==='create' ? 'Guardar' : 'Actualizar'"></button>
            </div>
          </form>
        </div>
      </div>
      ...
      {{-- Tabla --}}
      <div class="overflow-x-auto bg-gray-800 text-white rounded">
            ...
            @foreach($clientes as $c)
              <tr class="border-t border-gray-700">
                ...
                <td class="px-4 py-2 space-x-2">
                  ...
                  <form method="POST" action="{{ route('gestion_clientes.destroy', $c->id) }}"
    ...
    <script>
        function clienteManager() {
          return {
            ...
            form: {
              id: null,
              ciudad_id: '', sector_id: '', manzana_id: '',
              nombre: '', direccion: '', telefono: '', email: '',
              crear_medidor: false, medidor_codigo: '', medidor_fecha_instalacion: '',
              ubicacion_detallada: '', tarifa_id: '', consumo_sin_medidor_id: '',
              categoria: '', code_suministro: ''
            },

            filterSectores() {
              this.sectores = this.allSectores.filter(s =>
                s.reservorio?.bomba?.id_ciudades == this.form.ciudad_id
              );
              this.form.sector_id = '';
              this.manzanas = [];
            ...
            updateCode() {
              if (!this.form.ciudad_id || !this.form.sector_id || !this.form.manzana_id) {
              ...
              const d = new Date(),
                    ...
                    fecha = `${y}${m}${day}`;
              this.form.code_suministro = `${this.form.ciudad_id}-${this.form.sector_id}-${this.form.manzana_id}-${fecha}`;
            },
            ...
            openForm(mode, cliente = null) {
              ...
              if (mode === 'create') {
                ...
                // Usar optional chaining para evitar errores si falta alguna relación
                this.sectores = this.allSectores.filter(s =>
